...
3. Click "Load unpacked" and pick `apps/extension-flatscout/dist/chrome/`.

## Install (Firefox)

1. Visit `about:debugging` → "This Firefox" → "Load Temporary Add-on".
2. Pick `apps/extension-flatscout/dist/firefox/manifest.json`.
3. **Grant optional host permissions:** open `about:addons` → Flatscout Bridge → Permissions, enable each of `homegate.ch`, `api.homegate.ch`, `immoscout24.ch`, `api.immoscout24.ch`. Firefox does not auto-grant MV3 host permissions; without these the bridge cannot `executeScript` against the target tabs.

(Firefox temporary add-ons unload on browser restart — re-load each session until the extension is signed.)

## Pair

1. Make sure `flatscout start` is running with `bridge.enabled: true` in `config.yaml`.
2. Run `flatscout bridge pair` — it prints the WebSocket URL and a 64-char hex token.
3. Open the extension popup, paste both, click "Save & connect".
4. The popup should switch to "connected".
5. `flatscout bridge status` should report `connected on port 8431; extension last seen Ns ago`.
...
- **Single tenant.** Only one extension may be paired at a time per Flatscout agent. A newer pairing preempts the older one server-side.
- **127.0.0.1 only.** The bridge is not reachable from other machines on the LAN.
- **Headless deployments.** Flatscout falls back to its Playwright transport automatically when no extension is paired. The bridge is only useful where you'd otherwise be fighting DataDome from a server.
...
1. Build: `FLATSCOUT_EXT_BROWSER=chrome pnpm --filter @flatscout/extension build`
2. `chrome://extensions` → Developer mode → "Load unpacked" → `apps/extension-flatscout/dist/chrome`
3. Click the extension's action icon; popup opens.
4. Run `flatscout bridge pair` in a terminal; copy the URL + 64-hex token into the popup; click Save.
...
7. Start `flatscout start` if not running.
8. `flatscout doctor` — expect bridge `connected: true`.
9. Wait 5 minutes. `flatscout doctor` again — still `connected: true`.
10. Trigger a Homegate scan: `flatscout scan --source source-homegate`. Listings persisted.
...
1. Build: `FLATSCOUT_EXT_BROWSER=firefox pnpm --filter @flatscout/extension build`
2. `about:debugging` → "This Firefox" → "Load Temporary Add-on" → pick `manifest.json` in `apps/extension-flatscout/dist/firefox`
3. Pair as for Chrome.
4. **Grant host permissions in `about:addons`** (Firefox does not auto-grant MV3 host permissions).
5. Confirm `flatscout doctor` reports bridge connected within 90 s.

## Manual smoke test (cross-process)

With `flatscout start` daemon running + extension paired:

1. In a second terminal: `flatscout scan --source source-homegate`.
2. Listings persist; no Playwright fallback messages in logs.
3. `flatscout doctor` reports bridge `inflight: 0` after the scan completes.
