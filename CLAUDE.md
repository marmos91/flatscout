...
- `packages/server` — orchestrator: plugin loader (dynamic `import` by name + Zod validation), pipeline, node-cron scheduler, daily quota gate, per-source circuit-breaker, shutdown-hook lifecycle. Starts `@wabe/browser-bridge` when `top.bridge.enabled` is true. Persistence model: one row per logical listing keyed by `canonical_key` — `packages/server/src/dedupe.ts:mergeUpsertCanonical` folds second-source observations into the existing row via `@wabe/core/resolveFields` (priority-wins with null-skip; photos union; enriched deep-merge). Notification fires only on the first canonical-row INSERT.
